...
class AddSettlementDialog extends ConsumerStatefulWidget {
  final String projectId;
  const AddSettlementDialog({required this.projectId, super.key});

  @override
  ConsumerState<AddSettlementDialog> createState() => _AddSettlementDialogState();
}

class _AddSettlementDialogState extends ConsumerState<AddSettlementDialog> {
    ...
    try {
      ...
      if (_proofImage != null) {
        ...
        proofUrl = await storage.uploadFile(
          compressedPath,
          'projects/${widget.projectId}/settlement_proofs',
        );
      }
      ...
      final settlement = SettlementEntity(
        ...
        date: _selectedDate.toLocal().toString().split(' ')[0], // simple date string
        paidTo: _paidToController.text.trim(),
        paymentMethod: _paymentMethodController.text.trim(),
        proofUrl: proofUrl,
      );
      
      final success = await ref.read(addSettlementNotifierProvider.notifier).addSettlement(widget.projectId, settlement);
      ...
      if (success && mounted) {
        ...
        try {
          ...
          await ns.notifyAdminOfSettlement(
            projectId: widget.projectId,
            projectName: project?.projectName ?? '',
            senderName: meta['name'] as String? ?? 'Manager',
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Settlement added successfully')));
      } else if (mounted) {
        final state = ref.read(addSettlementNotifierProvider);
        ...
        children: [
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(
              labelText: 'Payment Method (e.g., Bank Transfer, Check)',
